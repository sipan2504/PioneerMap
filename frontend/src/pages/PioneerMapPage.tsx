...
> = {
  Stays: { icon: "🏠", color: "#1988FF" },
  Shops: { icon: "🛍️", color: "#FF2AA6" },
  Food: { icon: "🍴", color: "#FFB21C" },
  Services: { icon: "🔧", color: "#13B39B" },
  Jobs: { icon: "💼", color: "#8D2CFF" },
};
...
function PioneerMapPage() {
  ...
  const toggleFavorite = (place: Place) => {
    ...
    setStatus(
      isFavorite(place)
        ? `⭐ ${extraTranslations[appLanguage].removeFavorite}`
        : `⭐ ${extraTranslations[appLanguage].addFavorite}`
    );
  };
  ...
  const sharePlace = async (place: Place) => {
    ...
    try {
      if (navigator.share) {
        ...
        setStatus(`📤 ${extraTranslations[appLanguage].share}`);
        return;
      }

      await navigator.clipboard.writeText(shareUrl);
      setStatus(`📋 ${appLanguage === "English" ? "Place link copied." : "Bağlantı kopyalandı."}`);
    } catch {
      try {
        await navigator.clipboard.writeText(shareUrl);
        setStatus(`📋 ${appLanguage === "English" ? "Place link copied." : "Bağlantı kopyalandı."}`);
      } catch {
        setStatus(`❌ ${appLanguage === "English" ? "The place link could not be copied." : "Paylaşım bağlantısı kopyalanamadı."}`);
      }
    }
  };
  ...
  const deletePlace = async (
    ...
    if (!place._id) {
      setStatus(
        `❌ ${appLanguage === "English" ? "This sample place cannot be deleted." : "Bu örnek kayıt silinemez."}`
      );
      return;
    }
  ...
  const locateUser = () => {
    if (!navigator.geolocation) { setStatus(t("locationNotSupported")); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const map = mapInstance.current;
        if (map) {
          map.setView([next.lat, next.lng], 14, { animate: true });
          window.setTimeout(() => map.invalidateSize(), 120);
        }
        setStatus(t("locationFound"));
      },
...
.pm-main{padding:18px 18px 34px;background:linear-gradient(180deg,#08234b 0%,#071c3c 48%,#061a38 100%)}
.pm-map-card{position:relative;overflow:hidden;border:1px solid rgba(88,177,255,.42);border-radius:20px;background:#123b60;box-shadow:0 20px 48px rgba(0,0,0,.34)}
.pm-map{height:610px;width:100%;transition:height .28s ease;background:#1b4c70 url("data:image/jpeg;base64,") center/cover no-repeat}.pm-map-card.collapsed{min-height:70px}.pm-map-card.collapsed .pm-map{height:0;min-height:0}
...
.pm-bottom{position:fixed;left:0;right:0;bottom:0;z-index:10000;display:grid;grid-template-columns:repeat(5,1fr);padding:8px 10px calc(8px + env(safe-area-inset-bottom));background:rgba(5,27,61,.98);border-top:1px solid rgba(112,184,203,.16);box-shadow:0 -14px 32px rgba(0,0,0,.30);backdrop-filter:blur(18px)}.pm-nav{border:0;background:transparent;color:#7896a3;font-size:10px;font-weight:850;cursor:pointer;padding:5px 2px;border-radius:12px}.pm-nav.active{color:#e8c45b;background:rgba(232,196,91,.08)}.pm-nav span{display:block;font-size:21px;margin-bottom:3px}.pm-add{width:54px;height:54px;margin:-28px auto 0;display:grid!important;place-items:center;border:4px solid #061522;border-radius:50%;background:linear-gradient(145deg,#1995ff,#0875ea);color:#fff;font-size:31px;box-shadow:0 8px 24px rgba(0,0,0,.35)}
.pm-overlay-panel{position:fixed;inset:0;bottom:78px;z-index:9000;overflow-y:auto;background:#06111f;padding:14px}.pm-overlay-close{position:sticky;top:0;float:right;z-index:2;width:42px;height:42px;border:1px solid rgba(129,203,220,.28);border-radius:12px;background:#0a2740;color:#fff;font-size:28px;line-height:1;cursor:pointer}
        ...
        <div className="pm-shell">
          <div className="pm-header-row">
            ...
            <div className="pm-actions">
              ...
              <button type="button" className="pm-pill" onClick={loginWithPi}>{signedIn ? `✓ @${username}` : (appLanguage === "English" ? "π Pi Sign In" : t("signIn"))}</button>
            </div>
          </div>
        ...
        <div className="pm-shell">
          <section className={`pm-map-card ${showMap ? "" : "collapsed"}`}>
            ...
            <div className="pm-map-overlay">
              {showMap && <button type="button" className="pm-map-locate" onClick={locateUser} aria-label={appLanguage === "English" ? "My location" : extraTranslations[appLanguage].location}>◎</button>}
              {showMap ? <div className="pm-map-controls"><button type="button" className="pm-map-control" onClick={()=>{setShowMap(false);window.setTimeout(()=>mapInstance.current?.invalidateSize(),180)}}>◉ {appLanguage === "English" ? "Hide Map" : appLanguage === "Turkish" ? "Haritayı Gizle" : extraTranslations[appLanguage].showOnMap}</button><button type="button" className="pm-map-control nearby" onClick={findNearbyPlaces}>➤ {appLanguage === "English" ? "Nearby Places" : t("nearby").replace("📍 ","")}</button></div> : <div className="pm-map-controls"><button type="button" className="pm-map-control active" onClick={()=>{setShowMap(true);setMapInteractive(true);window.setTimeout(()=>mapInstance.current?.invalidateSize(),180)}}>◉ {appLanguage === "English" ? "Show on Map" : appLanguage === "Turkish" ? "Haritada Göster" : extraTranslations[appLanguage].showOnMap}</button></div>}
            </div>
          </section>
          ...
          <section className="pm-user-news">
            <div className="pm-user-card" id="pioneer-profile-card"><div className="pm-user-row"><div className="pm-avatar" /><div><strong>{signedIn ? `@${username}` : "Dilek"}</strong><span>{signedIn ? t("connected") : (appLanguage === "English" ? "Pioneer • Explorer" : "Pioneer")}</span></div><span className="pm-arrow">›</span></div></div>
            <a className="pm-news-card" href={activeNews.url} target="_blank" rel="noreferrer"><div className="pm-news-row"><div className="pm-news-icon" /><div style={{minWidth:0,flex:1}}><strong>{appLanguage === "English" ? "Pi News" : appLanguage === "Turkish" ? "Pi Haberleri" : "Pi News"}</strong><span>{activeNews.title}</span></div><span className="pm-arrow">›</span></div></a>
          </section>

          <section className="pm-news-feed"><div className="pm-news-head"><strong>🟣 {appLanguage === "English" ? "Pi News" : appLanguage === "Turkish" ? "Pi Haberleri" : "Pi News"}</strong><a href="https://minepi.com/blog/" target="_blank" rel="noreferrer">{appLanguage === "English" ? "Official Pi updates →" : appLanguage === "Turkish" ? "Resmî Pi güncellemeleri →" : "Pi News →"}</a></div><div className="pm-news-list">{piNews.map((item,index)=><a key={item.url} className="pm-news-item" href={item.url} target="_blank" rel="noreferrer"><span className="pm-news-badge">P</span><span className="pm-news-copy"><strong>{index === newsIndex % piNews.length ? "● " : ""}{item.title}</strong><span>{item.summary}</span></span><span className="pm-news-date">{item.date}</span></a>)}</div></section>
